...
mystudies <- unique(Mat_Data_Targeted %>% 
    mutate(Study =  str_remove(Study, '[_]\\w+|:')) %>% 
  pull(Study))
...
for (i in seq_along(mystudies)) {
  ...
  for (j in seq_along(curr_demo_features)) {
    ...
    Plot_Dfs_Mat[[key]] <- DF_plot
    

  }
}

save(Plot_Dfs_Mat,file = "../../../HHEAR_Man1_App/Mat_SDOH_Plot_Dfs.RData")
```

```{r}
myanalytes <- unique(Final_Intervals_Df_Adjusted$Outcome)

mystudies <- unique(Child_Data_Targeted %>% 
    mutate(Study =  str_remove(Study, '[_]\\w+|:')) %>% 
  pull(Study))
...
for (i in seq_along(mystudies)) {
  ...
  for (j in seq_along(curr_demo_features)) {
    ...
    plot_idx <- plot_idx + 1
    key <- paste0(mystudies[[i]], "__", curr_demo_features[[j]])
    Plot_Dfs_Child[[key]] <- DF_plot

  }
}

save(Plot_Dfs_Child,file = "../../../HHEAR_Man1_App/Child_SDOH_Plot_Dfs.RData")
...
pdf(file = "../../../Output/Paper1_OutputV2/Plots/Mat_Facet_SDOH_Intervals.pdf", height = 16, width = 16)
...
unique(Child_Data_Targeted$Study)

plots <- list()            # collect everything here
plot_idx <- 0

pdf(file = "../../../Output/Paper1_OutputV2/Plots/Child_Facet_SDOH_Intervals.pdf", height = 16, width = 16)
...
```{r}
#save(plots_child, file = "../../../HHEAR_Man1_App/Child_SDOH_Plots.RData")
#save(plots_mat, file = "../../../HHEAR_Man1_App/Mat_SDOH_Plots.RData")
...
madres_plots <- plots
save(madres_plots, file = "../../../Output/Paper1_OutputV2/madres_plots.RData")
